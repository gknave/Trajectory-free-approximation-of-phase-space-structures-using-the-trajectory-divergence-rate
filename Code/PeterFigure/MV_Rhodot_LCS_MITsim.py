...
root1 = Dataset(ncfile,'r') #read the data
variables = root1.variables #dictionary, all variables in dataset\
...
lat = variables["lat"][:]
lon = variables["lon"][:]
time = 86400*variables["time"][:]+tstart
u = variables["eastward_vel"][t,:,:]
v = variables["northward_vel"][t,:,:]
...
root2 = Dataset(ncfile,'r') #read the data
variables = root2.variables #dictionary, all variables in dataset\
aftle = variables['FTLE'][:,aftlesnap,0]
flon = variables['initial_lon'][:]
flat = variables['initial_lat'][:]
ydim = 479
xdim = 525
acg = variables['CauchyGreen'][:,aftlesnap,:,:]
...
root3 = Dataset(ncfile,'r') #read the data
variables = root3.variables #dictionary, all variables in dataset\
rftle = variables['FTLE'][:,rftlesnap,0]
rcg = variables['CauchyGreen'][:,rftlesnap,:,:]
...
'''
ax = plt.subplot(111)
heatmap = m.contourf(lon,lat,rhodot,levels=np.linspace(-colorlevel,colorlevel,301),vmin=-0.25*colorlevel,vmax=0.25*colorlevel,cmap = 'CO',latlon=True)#,shading='gourand')
clb = plt.colorbar(heatmap, cax=None, use_gridspec=True, ticks=[-1, -0.5, 0, 0.5, 1])
...
fig = plt.figure(2,figsize=(3,1.4), dpi=150)
#m.scatter(lon[153,:],lat[153,:],latlon=True)
plt.plot(lon[153,:],rhodot[153,:],color=(0, 0.327, 0.327))
plt.plot(lon[153,:],np.zeros(lon[153,:].shape), color=(0.2,0.2,0.2), linewidth=0.4)
plt.plot(flon[306,:],aftle[306,:],color='blue')
plt.plot(flon[306,:],rftle[306,:],color='red')
plt.xlim([-70.6, -70.3])
plt.ylim([-2.8, 1.8])
plt.yticks([-2, -1, 0, 1], fontsize=8, family='serif')
plt.xticks([-70.6, -70.5, -70.4, -70.3], fontsize=8, family='serif')
plt.ylabel(r'$\dot{\rho}$', fontsize=10, family='serif')
...
variables = root.variables #dictionary, all variables in dataset\
print variables.keys()
t=14
iterplevl = 24
dx=200 #m
